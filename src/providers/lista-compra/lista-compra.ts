...
/*
  Generated class for the ListaCompraProvider provider.
...
export class ListaCompraProvider {
  ...
  itemsRef: AngularFireList<any>;
    ...
    constructor(protected db: AngularFireDatabase, private authProvider: AuthProvider) {
      this.itemsRef = db.list('lista-compra'); //seleciona qual tabela será conectado
    }
    ...
    retornaUm(id):any {
      return this.db.object( 'lista-compra/'+ id).valueChanges();
    }

      // retorna um observable real time da lista de local
  getAll() {
    return this.db.list('lista-compra').snapshotChanges();
  }

  getAllSubscribed() {
    //return this.db.list('lista-compra/usuarioscompartilhado', ref => ref.orderByChild('id').equalTo(this.authProvider.afAuth.auth.currentUser.uid)).snapshotChanges();
    return this.db.list('lista-compra',ref => {
      return ref.orderByChild('usuarioscompartilhado').equalTo(this.authProvider.afAuth.auth.currentUser.uid);
    }).snapshotChanges();
  }
